...
def MAC(inputQ, weightQ, abits, wbits, adcbits, output_size, subArray, clamp=1.):
    ...
    for z in range(abits):
        ...
        for k in range(wbits):
            ...
            outputPartial = torch.clamp(outputPartial, 0, subArray)
    ...
    return outputShiftIN, quant_err, out_mean.item(), out_var.item()
...
class QConv2dCAM(nn.Conv2d):
    ...
    def __data_update__(self, index, search_time, search_cnt, quant_err, out_mean, out_var):
        ...
        self.out_var[f"{self.subArray}_{index}"] = out_var
    ...
    def forward(self, input):
        ...
        if self.inference == 3:
            if numSubArray == 0:
                ...
                outputL, quant_err, out_mean, out_var = MAC(inputQ, weightQL, self.bitActivation, self.bitWeightLSB, self.bitADC, outputreal.size(), weight2D.shape[1], self.ADClamp)
                ...
                self.__data_update__(self.subArray, search_time, search_cnt, quant_err, out_mean, out_var)
                ...
                for s, rowArray in enumerate(numSubRow):
                    ...
                    outputM, search_time, search_cnt = CAM(inputQ, weightQM*mask, self.bitActivation, self.bitWeightMSB, self.bitADC, outputreal.size(), rowArray)
                    outputL, quant_err, out_mean, out_var = MAC(inputQ, weightQL*mask, self.bitActivation, self.bitWeightLSB, self.bitADC, outputreal.size(), rowArray, self.ADClamp)
                    ...
                    self.__data_update__(index, search_time, search_cnt, quant_err, out_mean, out_var)
